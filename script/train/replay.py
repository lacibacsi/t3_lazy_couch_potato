...
'''
    reused from: https://github.com/MichaelBosello/f1tenth-RL    
    sampling and replay buffer implementation
'''
...
class ReplayMemory:
    ...
    def __init__(self, base_output_dir, model=None):
        ...
        self.samples = []
        self.max_samples = 'store_true'  # args.replay_capacity
        self.prioritized_replay = 'store_true'  # args.prioritized_replay
        self.num_interesting_samples = 0
        self.batches_drawn = 0

        if model is not None:
            self.load(model + self.file)
    ...
    def draw_batch(self, batch_size):
        if batch_size > len(self.samples):
            raise IndexError('Too few samples (%d) to draw a batch of %d' % (
                len(self.samples), batch_size))
    ...
    def _truncate_list_if_necessary(self):
        # optimisation: don't truncate on each added sample since it requires a memcopy of the list
    ...
    def _draw_prioritized_batch(self, batch_size):
        ...
        while len(batch) < batch_size:
            probe.cumulative_weight = random.uniform(
                0, self.samples[-1].cumulative_weight)
            index = bisect.bisect_right(
                self.samples, probe, 0, len(self.samples) - 1)
    ...
    def _update_weights(self):
        if len(self.samples) > 1:
            self.samples[-1].cumulative_weight = self.samples[-1].weight + \
                self.samples[-2].cumulative_weight
        ...
        if self.samples[-1].is_interesting():
            ...
            uninteresting_sample_range = max(
                1, len(self.samples) / max(1, self.num_interesting_samples))
            ...
            for i in range(uninteresting_sample_range, 0, -1):
                ...
                boost = 1.0 + 3.0 / \
                    (math.exp(i/(uninteresting_sample_range/6.0)))
                self.samples[index].weight *= boost
                self.samples[index].cumulative_weight = self.samples[index].weight + \
                    self.samples[index - 1].cumulative_weight
