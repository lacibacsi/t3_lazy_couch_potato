...
class State:
    '''
        State class taken from https://github.com/MichaelBosello/f1tenth-RL/blob/master/f1tenth-rl/state.py
        simplified for LIDAR only
    '''
    ...
    def process_data(self, data):
        ...
        if State.lidar_reduction_method == 'avg':
            ...
            for i in range(0, len(data), State.reduce_by):
                filtered = list(
                    filter(lambda x:  x <= State.max_distance_norm, data[i:i + State.reduce_by]))
                if len(filtered) == 0:
    ...
    def lidar_to_img(self, data):
        img_array = np.zeros(
            (State.image_width, State.image_height), dtype=np.uint8)
