...
# from openai_ros.task_envs.TurtleBot3WorldEnv import stay_up
...
if __name__ == '__main__':
    ...
    pkg_path = rospack.get_path('t3_lazy_couch_potato')
    ...
    epsilon_discount = rospy.get_param(
        "/t3_lazy_couch_potato_v0/epsilon_discount")
    nepisodes = rospy.get_param("/t3_lazy_couch_potato_v0/nepisodes")
    nsteps = rospy.get_param("/t3_lazy_couch_potato_v0/nsteps")
    running_step = rospy.get_param("/t3_lazy_couch_potato_v0/running_step")
    ...
    for x in range(nepisodes):
        ...
        h, m = divmod(m, 60)
        rospy.logwarn(("EP: "+str(x+1)+" - [alpha: "+str(round(qlearn.alpha, 2))+" - gamma: "+str(round(qlearn.gamma, 2))+" - epsilon: "+str(
            round(qlearn.epsilon, 2))+"] - Reward: "+str(cumulated_reward)+"     Time: %d:%02d:%02d" % (h, m, s)))
